...
type AuditSummary = {
  ...
  validObjectIdClientReferences: number;
  missingClientId: number;
  malformedClientIdentifiers: number;
  deletedOrNonexistentClients: number;
  crossUserClientReferences: number;
  legacyClientFields: number;
  validStringObjectIdsConvertible: number;
  manualReviewRequired: number;
  convertibleProjectIds: string[];
  manualReviewProjectIds: string[];
};
...
const ensureRecentDryRun = () => {
  ...
  const report = JSON.parse(fs.readFileSync(auditFile, "utf8")) as { createdAt: string };
...
const markManualReview = (summary: AuditSummary, projectId: string) => {
...
const main = async () => {
  ...
  for (const project of projects) {
    ...
    if (hasLegacyClient) summary.legacyClientFields += 1;
  ...
  if (apply) {
    const result = await Project.collection.updateMany(
      { _id: { $in: summary.convertibleProjectIds.map((id) => new mongoose.Types.ObjectId(id)) } },
      [{ $set: { clientId: { $toObjectId: "$clientId" } } }]
    );
